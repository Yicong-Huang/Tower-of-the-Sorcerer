...
def set_image(item):
    ...
    image=lib[item._name].image
    name=lib[item._name].name
    file='/'.join(['src']+[cls.__name__ for cls in inspect.getmro(item.__class__)][::-1][3:]+[name+'.png'])
    
    if image==1:
        lib[item._name]=lib[item._name]._replace(image=PhotoImage(file=file))
    elif image==2:
        lib[item._name]=lib[item._name]._replace(image=(PhotoImage(file=file),PhotoImage(file=file[:-4]+'2.png')))
    elif isinstance(image, str):
        lib[item._name]=lib[item._name]._replace(image=lib[image].image)
