        ...
        if self._useable:
            if self._name == 'info_book':
                ...
                def search_stairs(prot):
                    return any(type(prot.get_value('floor').get_floor(prot._i + i, prot._j + j)).__name__ == 'Build'
                               and 'stairs' in prot.get_value('floor').get_floor(prot._i + i, prot._j + j)._name
                               for i in [-1, 0, 1] for j in [-1, 0, 1])

                if search_stairs(prot) and event.y <= 208 and prot.get_value('tower').check_active('up'):
                    prot.set_value('floor', prot.get_value(
                        'tower').next_floor())
                    print(prot._floor.num)
                ...
                prot._tower._current = prot._tower._tower[
                    prot._tower.get_num(prot._tower._current) - 1]
                # self._useable=False
                ...
                self.selecting_remove(
                    prot, ('great_magic', 'fake_lord1', 'fake_lord2', 'dragon'), mode=False, Type='Mob')
                # self._useable=False
